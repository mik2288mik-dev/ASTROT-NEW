...
Forecast copy is clean and readable: a short shared heading and at most 150 words in total. Today reads top-to-bottom as 4–6 untitled fragments with no visible Love/Work/Mood categories; Week and Month remain single stories. The personal visual source contains 309 assets under `/assets/personal-editorial/`: 202 `editorial-v2` assets, 45 cats, 38 capybaras, and 24 objects. Another 19 empty templates under `/assets/personal-paper-templates/` support live runtime text. Embedded-text and review-excluded assets remain catalogued but never enter automatic selection.

The Zodiac-only legacy source is physically and logically separate: `/assets/zodiac-legacy-special/` contains 48 explicitly allowlisted assets, split into 24 psychedelic images and 24 approved funny-animal images. `lib/personalForecastVisuals/editorialSelectors.ts` reads only the personal and paper manifests; `lib/zodiacLegacyVisuals/index.ts` reads only the Zodiac manifest backed by its typed allowlist.

Selection uses `userId + periodKey + contractVersion`, honours rarity, weight, orientation, and layout compatibility, and never uses random state. Luna never chooses assets, layout, coordinates, or colours. Some days are image-free, every day has at most one strong visual, and adjacent days do not repeat layout or asset. Body prose is never placed over imagery or inside a card. A paper note is the explicit exception: runtime text on an empty paper template, never text baked into PNG/WebP. Do not add promo banners, feedback prompts, questions, “hit/miss” controls, chat, games, or morning/day/evening segmentation without an explicit product decision.
