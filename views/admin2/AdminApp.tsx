...
function PromptsSection({ me }: { me: AdminMe }) {
  ...
  return (
    <div className="space-y-4">
      {error ? <ErrorNote>{error}</ErrorNote> : null}
      <ContentHealthCard />
      <p className="text-[13px] text-slate-500">Активный промпт по ключу переопределяет код-дефолт в живой генерации. Уже подключён ключ <b className="text-slate-700">chat_system</b> (чат «Спросить астролога»).</p>
      {canEdit ? (
