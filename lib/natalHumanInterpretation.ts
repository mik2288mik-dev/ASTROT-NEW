...
const HUMAN_SYSTEM_PROMPT = `Ты пишешь для Lumia разбор натальной карты на русском языке. Пиши как умный, тёплый и наблюдательный человек, который объясняет простыми словами — будто разбираешь характер близкого друга, а не зачитываешь гороскоп.

Главное правило: переводи расчёты по дате, времени и месту рождения в обычный человеческий язык. Читатель не знает астрологии и не должен её знать, чтобы тебя понять.
...
ЗАПРЕЩЕНО: космос, Вселенная, энергия и энергетика, вибрации, частоты, чакры, аура, карма, кармический, судьба, предначертано, миссия, предназначение свыше, духовный путь, высшие силы, тонкие материи, астрал, поток изобилия; мистика и эзотерика; фатальные и пугающие формулировки; предсказания конкретных событий; медицинские и психиатрические диагнозы; поэтичные абстракции и «инстаграм-психология»; вода и общие фразы, которые подходят кому угодно. Это «эзотерический туман», которого быть не должно.
...
export function buildHumanDailyFallback(
...
): InterpretationSection {
  {
    ...
    // Каждая сфера — самостоятельный текст СО СВОИМ примером и без «системного» вступления
    // про Солнце/Луну (из-за него все вкладки выглядели одинаково и шаблонно).
    const specific: Record<HumanDailySectionKey, string> = {
      daily_overview: 'Сегодня день охотнее отзывается на собранность, чем на попытку успеть всё сразу. Выбери одно главное дело и держись его, не растягиваясь на чужие срочные запросы — иначе к вечеру окажется, что сил ушло много, а сдвинулось мало.\n\nНапример: с утра сыплются сообщения и просьбы. Возьми одну свою задачу и доведи её до конца, остальное — по остаточному принципу. Спокойнее к вечеру ты будешь не потому, что сделал больше, а потому что сделал главное и не дал растащить себя по мелочам.',
      daily_work_business: 'В работе сегодня важнее довести до конца один участок, чем красиво начать пять. Ощущение занятости легко перепутать с движением: вкладок открыто десять, а реально закрыто ноль. Сила дня — в том, чтобы выбрать, что именно ты сегодня доведёшь до результата.\n\nНапример: дело стоит из-за «непонятно, кто за что отвечает». Закрой именно этот вопрос — короткое сообщение или звонок, — а не хватайся за новую задачу поверх застрявшей. Один доведённый до конца кусок даёт больше спокойствия, чем длинный список начатого.',
      daily_love: 'В близких отношениях сегодня больше даёт прямота, а не намёки. Если внутри копится «должен(на) сам(а) догадаться» — именно из этого вырастает большинство мелких обид. Скажи простыми словами: что произошло, что задело и чего ты хочешь дальше. Это не конфликт, это ясность.\n\nНапример: человек обещал ответить и снова тянет. Не прокручивай это весь день и не наказывай молчанием — спокойно спроси: «Когда сможешь дать точный ответ?» Один честный вопрос сегодня экономит вечер, который иначе уйдёт на додумывание и тихое раздражение.',
      daily_money: 'В деньгах сегодня выигрывает трезвость: проверяй условия, сроки и реальную нужность, прежде чем платить. Большая часть импульсивных трат — это попытка быстро успокоить эмоцию, а не закрыть настоящую потребность, и завтра за это приходится доплачивать сожалением.\n\nНапример: тянет купить «чтобы стало легче» — отложи до вечера и сравни хотя бы два варианта. Если к вечеру вещь всё ещё нужна, решение будет крепким; если отпустило — ты сохранил и деньги, и спокойствие. Сегодня пауза перед оплатой стоит дороже самой покупки.',
      daily_goals: 'Для целей сегодня работает не масштаб, а завершённость. Большой план, который весь день крутится в голове, утомляет и создаёт ощущение, что ты ничего не успеваешь — хотя на деле просто не выбрал, с чего начать.\n\nНапример: не «разобраться со всем проектом», а сделать первый конкретный кусок и поставить галочку. Один видимый завершённый шаг возвращает чувство, что делами управляешь ты, а не они тобой. Дроби большое на куски — и сегодня закрой ровно один.',
      daily_communication: 'В разговорах сегодня помогает пауза: сначала пойми, что именно хочешь сказать, и только потом усиливай голос. Меньше намёков и проверок — больше прямых, спокойных формулировок. Часто конфликт рождается не из темы, а из тона и недосказанности.\n\nНапример: вместо «ну ты сам понимаешь» скажи прямо, что тебе нужно и к какому сроку. Прямая просьба звучит мягче, чем долгий намёк, и почти всегда быстрее приводит к нужному ответу.',
      daily_friendship: 'В контактах сегодня выбирай людей, после которых становишься собраннее, а не пустее. Не обязательно отвечать всем и сразу — внимание тоже ресурс, и его стоит тратить туда, где оно возвращается теплом, а не усталостью.\n\nНапример: если переписка по кругу вытягивает силы — поставь её на паузу без чувства вины и вернись, когда будешь в ресурсе. Один тёплый честный разговор сегодня важнее десяти дежурных.',
      daily_family: 'Дома сегодня важны простые опоры: порядок, спокойный тон и отсутствие лишних драм. Часто силы возвращает не большой разговор, а закрытый бытовой вопрос и понятное правило, о котором договорились без надрыва.\n\nНапример: реши один отложенный домашний момент или мягко проговори договорённость на ближайшие дни. Маленькое наведение порядка в пространстве нередко наводит порядок и внутри.',
      daily_energy: 'По нагрузке сегодня лучше не брать всё сразу — оставь место для паузы и одного спокойно завершённого дела. Тревога часто гонит делать быстрее, чем реально нужно, и именно эта спешка съедает силы, а не сам объём задач.\n\nНапример: между делами дай себе короткую честную паузу без телефона. Ритм «сделал — выдохнул — пошёл дальше» сегодня сбережёт больше энергии, чем попытка пробежать день на одном рывке.',
      daily_risks: 'Главный риск дня — ответить слишком быстро или взять на себя лишнее на эмоциях. Проверь, где ты действуешь из ясности, а где из внутреннего напряжения и желания поскорее закрыть дискомфорт.\n\nНапример: прежде чем согласиться или резко ответить, дай себе одну паузу на вдох. Часто именно эта секунда отделяет решение, за которое завтра скажешь себе спасибо, от того, которое придётся исправлять.',
      daily_best_action: 'Лучшее действие дня — выбрать одну давно отложенную тему и сделать по ней спокойный, видимый шаг. Не обязательно закрывать её целиком; важно сдвинуть с мёртвой точки то, что давно висит фоном и тихо забирает внимание.\n\nНапример: один звонок, одно письмо или один абзац того, что ты откладывал. После этого день ощущается иначе — как будто внутри стало просторнее.',
      daily_advice: 'Не копи раздражение — называй факт и следующий шаг, пока эмоция не превратилась в стену. Сегодня сила не в том, чтобы продавить, а в том, чтобы найти дверь: паузу, точное слово или прямой вопрос.\n\nНапример: вместо того чтобы спорить, кто прав, спроси «что нам сейчас сделать, чтобы было лучше?» Этот вопрос разворачивает разговор из борьбы в решение.',
    };
    const bulletsByKey: Partial<Record<HumanDailySectionKey, string[]>> = {
      daily_love: ['Говори о фактах, не о догадках', 'Меньше проверок молчанием', 'Назови, чего хочешь дальше'],
      daily_money: ['Проверяй условия до оплаты', 'Не трать, чтобы успокоиться', 'Сравни хотя бы два варианта'],
      daily_work_business: ['Доведи один участок до результата', 'Уточни, кто за что отвечает', 'Не открывай пять дел сразу'],
      daily_goals: ['Один завершённый шаг', 'Дроби большое на куски', 'Не держи весь план в голове'],
    };
    ...
    return {
      ...
      isLocked: false,
      teaser: meta.teaser,
      content: specific[key],
      bullets: bulletsByKey[key] || ['Один главный шаг', 'Меньше догадок', 'Больше конкретики'],
      ctaLabel: '',
    };
  }
  const summary = buildChartSummary(profile, chart);
