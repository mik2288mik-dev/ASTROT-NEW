...
function buildBasePrompt(summary: ChartSummary): string {
  return `Создай бесплатный базовый разбор личности по натальной карте — это «портрет человека», то, что читатель видит бесплатно. Он должен быть полноценным: чтобы было что почитать, а не пара общих фраз.
...
- "shortCard": { "title": "Если коротко", "text": "2–3 предложения — суть характера человеческим языком", "keywords": ["4–6 коротких слов-черт, без названий знаков и планет"], "advice": "одна тёплая практичная мысль" };
- "freeSections": массив ровно из 6 элементов в этом порядке и с этими ключами/заголовками (каждый ~120–180 слов, всего 700–950 слов):
  1. { "key": "base_portrait", "title": "Кто ты по сути", ... } — цельный портрет: характер, как принимаешь решения, как входишь в контакт. Синтез, а не перечисление планет.
  2. { "key": "strengths", "title": "Сильные стороны", ... } — на что человек реально может опираться, с примерами где это помогает.
  3. { "key": "growth_zones", "title": "Где бывает трудно", ... } — слабые места и зоны роста, мягко и честно, без диагнозов и запугивания.
  4. { "key": "emotional_world", "title": "Что у тебя внутри", ... } — эмоциональные потребности: что даёт покой, что выбивает из колеи, как восстанавливаешься.
  5. { "key": "how_others_see_you", "title": "Каким тебя видят", ... } — первое впечатление и как это отличается от того, какой человек внутри.
  6. { "key": "main_advice", "title": "Как с этим жить", ... } — 1 цельный практичный совет, как пользоваться своими сильными сторонами и обходить слабые.
  у каждой секции: "subtitle" (короткий, 2–4 слова), "access": "free", "content" (строка), "bullets" (2–3 коротких практичных вывода).
- "paidSections": [] и "premiumSections": [] — пустые массивы.

Требования к тексту:
- живой человеческий язык, конкретные примеры (работа, деньги, отношения, разговоры, решения, дом, отдых);
- НИКАКОЙ эзотерики, мистики и космизма — пиши как про реального человека (подробнее в системных правилах);
- опирайся на Солнце, Луну и Асцендент (Асцендент — только если время рождения надёжно);
- если время рождения неизвестно или неточно, мягко скажи об этом в секции "Каким тебя видят" (Асцендент может быть неточным), остальной портрет остаётся полезным;
- не повторяй одни и те же мысли в разных секциях, не дублируй абзацы.`;
}
  ...
  return generateWithRetry<NatalInterpretationReport>(
    async () => {
      const raw = await llmJson<NatalInterpretationReport>({
        ...
        // Базовый портрет — первое впечатление о продукте: генерим сильной моделью (deep-тир).
        user: prompt,
