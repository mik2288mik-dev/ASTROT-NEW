...
export function buildHumanDailyFallback(
...
): InterpretationSection {
  {
    ...
    const sun = summary.core.sun.sign;
    const moon = summary.core.moon.sign;
    // Каждая сфера — самостоятельный текст СО СВОИМ примером. Никакого общего вступления
    // и чужого примера про отношения в денежной/рабочей вкладке (из-за этого всё выглядело одинаково).
    const specific: Record<HumanDailySectionKey, string> = {
      daily_overview: 'Сегодня лучше выбрать одно главное дело и не распыляться на чужие срочные запросы. Например, если с утра валятся просьбы — возьми одну свою задачу и доведи её до конца, остальное по остаточному принципу.',
      daily_work_business: 'В работе полезно довести до ясности один участок: срок, договорённость или результат. Например, если дело стоит из-за «непонятно, кто за что отвечает» — закрой именно этот вопрос, а не берись за новое.',
      daily_love: 'В близком разговоре лучше говорить конкретно: что произошло, что задело и чего ты хочешь дальше. Например, если человек обещал ответить и снова тянет — не додумывай весь день, а спокойно спроси: «Когда сможешь дать точный ответ?»',
      daily_money: 'В деньгах сегодня лучше проверять условия и не покупать на эмоциях. Например, если тянет купить «чтобы успокоиться» — отложи до вечера: если вещь всё ещё будет нужна, решение окажется крепче.',
      daily_goals: 'Для целей подходит один короткий шаг, который реально завершить сегодня. Например, не «разобраться со всем проектом», а сделать первый конкретный кусок и поставить галочку.',
      daily_communication: 'В переговорах помогает пауза: сначала пойми, что хочешь сказать, и только потом усиливай голос. Меньше намёков — больше прямых формулировок.',
      daily_friendship: 'В контактах выбирай людей, после которых становишься собраннее, а не пустее. Сегодня не обязательно отвечать всем и сразу.',
      daily_family: 'Дома важны простые опоры: порядок, спокойный тон и без лишних драм. Закрой один бытовой вопрос или договорись о понятном правиле.',
      daily_energy: 'По нагрузке лучше не брать всё сразу — оставь место для паузы и одного завершённого дела. Тревога часто гонит делать быстрее, чем нужно.',
      daily_risks: 'Главный риск дня — ответить слишком быстро или взять на себя лишнее. Проверь, где ты действуешь из ясности, а где из напряжения.',
      daily_best_action: 'Лучшее действие дня — выбрать одну давно отложенную тему и сделать по ней спокойный видимый шаг.',
      daily_advice: 'Не копи раздражение: назови факт и следующий шаг. Не ломай стену — найди дверь, паузу или более точное слово.',
    };
    ...
    return {
      ...
      isLocked: false,
      teaser: meta.teaser,
      content: `Это твой день по карте (Солнце ${sun}, Луна ${moon}), а не общий гороскоп по знаку.\n\n${specific[key]}`,
