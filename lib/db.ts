...
function getPool(): Pool {
  if (!pool) {
    // Check ENV dynamically in case it was set after module load
    const currentDbUrl = process.env.DATABASE_URL || DATABASE_URL;
    ...
    const urlParts = currentDbUrl.match(/^postgres(ql)?:\/\/([^:]+):([^@]+)@([^:]+):(\d+)\/(.+)$/);
    if (urlParts) {
      const [, , user, , host, port, database] = urlParts;
...
export const db = {
  users: {
    async get(userId: string) {
      ...
      const dbPool = getPool();
      if (!dbPool) {
         // This should throw inside getPool if config is missing, but double check
         log.error('[DB] DATABASE_URL not configured');
         return null;
      }
