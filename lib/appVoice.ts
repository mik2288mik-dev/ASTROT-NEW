...
const APP_VOICE_MYSTICISM_PATTERNS: readonly RegExp[] = [
  /карм|чакр|астрал|эзотери|вселенн|мироздан|вибрац|сакральн|магич|предначертан|высшие\s+силы|тонкие\s+матери|духовн(?:ый|ого|ом)\s+пут/iu,
...
const APP_VOICE_CLICHE_PATTERNS: readonly RegExp[] = [
  ...
  /прислуша\w*\s+к\s+себе|доверь\w*\s+(?:себе|своему\s+пути|потоку)/iu,
  /позволь\w*\s+себе|отпусти\w*\s+контрол|будь\s+в\s+моменте/iu,
  /поберег\w*\s+ресурс|ресурсн(?:ое|ый|ая)\s+состояни|внутренн(?:ий|яя)\s+ресурс/iu,
  ...
  /проработ\w*|исцелени\w*|внутренн(?:ий|его)\s+реб[её]н/iu,
  ...
  /активн\w*\s+(?:тем|сфер|част|энерг)|что\s+из\s+этого\s+активно\s+сейчас|проявля\w*\s+сильнее/iu,
  /повторяющ\w*\s+(?:тем|сценари|паттерн)|внутренн\w*\s+рисунок|постоянн\w*\s+рисунок/iu,
