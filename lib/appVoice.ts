...
export const PERSONAL_FORECAST_VOICE_VERSION = '13';
...
const PERSONAL_FORECAST_VOICE_VIOLATION_PATTERNS: readonly RegExp[] = [
  ...
  /эмоциональн\p{L}*\s+(?:фон|состояни|ресурс|стабильност)\p{L}*|чувствительност\p{L}*\s+к\s+давлени\p{L}*|(?:твоя|ваша)\s+реакци\p{L}*|самочувстви\p{L}*|уверенност\p{L}*|раздражени\p{L}*/iu,
  ...
  /(?:^|[^\p{L}])(?:нажать|нажимай|нажми|жми|остановиться|остановись|останавливайся|выдох\p{L}*|прислушайся|не\s+распыляйся|не\s+распыляться)(?!\p{L})/iu,
...
const PERSONAL_FORECAST_BOOKISH_WORD_PATTERN = /(?:^|[^\p{L}])(?:первоначальн\p{L}*|предварительн\p{L}*|окончательн\p{L}*|облегчени\p{L}*|вполне|дальнейш\p{L}*)(?!\p{L})/iu;
 ...
 */
const PERSONAL_FORECAST_FORMAL_EVENT_PATTERN = /(?:^|[^\p{L}])(?:запланированн\p{L}*\s+(?:встреч\p{L}*|звонок\p{L}*|поездк\p{L}*)[^.!?]{0,90}назначенн\p{L}*\s+врем\p{L}*|нов\p{L}*\s+дат\p{L}*\s+вместо\s+(?:прежн|стар)\p{L}*|(?:на\s+)?экран\p{L}*\s+маршрут\p{L}*|расход\p{L}*[^.!?]{0,24}(?:будет|будут|оказал\p{L}*)\s+закрыт\p{L}*|(?:разговор|встреч)\p{L}*[^.!?]{0,30}(?:законч|заверш)\p{L}*\s+договор[её]нност\p{L}*)(?!\p{L})/iu;
...
const PERSONAL_FORECAST_WRITTEN_TIME_PATTERN = /(?:^|[^\p{L}])(?:намеченн\p{L}*\s+поездк\p{L}*[^.!?]{0,50}лишн\p{L}*\s+пересадк\p{L}*|освободивш\p{L}*\s+врем\p{L}*[^.!?]{0,35}не\s+раствор\p{L}*|отправ\p{L}*\s+ещ[её]\s+засветло)(?!\p{L})/iu;
...
const PERSONAL_FORECAST_ABSTRACT_INTEREST_PATTERN = /(?:^|[^\p{L}])(?:вариант\p{L}*\s+времени\s+для\s+разговор\p{L}*|совместн\p{L}*\s+договор[её]нност\p{L}*|иде\p{L}*\s+[^.!?]{0,24}(?:перейд|преврат)\p{L}*\s+в\s+[^.!?]{0,18}договор[её]нност\p{L}*|присоедин\p{L}*\s+к\s+уже\s+начат\p{L}*\s+дел\p{L}*)(?!\p{L})/iu;

const PERSONAL_FORECAST_WRITTEN_EVENT_PATTERN = /(?:^|[^\p{L}])(?:на\s+телефон\p{L}*\s+[^.!?]{0,22}прийти\s+[^.!?]{0,12}сообщени\p{L}*|обмен\p{L}*\s+детал\p{L}*|переписк\p{L}*\s+[^.!?]{0,24}привед\p{L}*\s+к\s+[^.!?]{0,18}встреч\p{L}*|работ\p{L}*\s+с\s+капающ\p{L}*\s+кран\p{L}*|мастер\p{L}*\s+верн\p{L}*\s+в\s+ванн\p{L}*|за\s+предел\p{L}*\s+привычн\p{L}*\s+круг\p{L}*|расшир\p{L}*\s+круг\p{L}*\s+заказчик\p{L}*|в\s+обсуждени\p{L}*\s+появ\p{L}*\s+(?:нов\p{L}*\s+)?(?:цифр\p{L}*|сумм\p{L}*)|человек\p{L}*\s+с\s+друг\p{L}*\s+сторон\p{L}*|появ\p{L}*\s+предложени\p{L}*\s+встрет\p{L}*\s+нескольк\p{L}*\s+люд\p{L}*|неприятн\p{L}*\s+добав\p{L}*)(?!\p{L})/iu;
