...
export function renderNatalStoryShareSvg(card: ProfileCard, format: NatalStoryShareFormat = 'story') {
  ...
  const svg = `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="84" y="${format === 'story' ? 166 : 132}" font-size="34" font-weight="800" letter-spacing="3" fill="${accent.a}">ТВОЙ ГОРОСКОП</text>
      <text x="84" y="${format === 'story' ? 222 : 182}" font-size="25" font-weight="600" letter-spacing="4" fill="#77717d">ЛИЧНАЯ КАРТА</text>
