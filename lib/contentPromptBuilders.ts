...
export function buildSynastryPrompt(input: PromptInput = {}): AppContentPrompt {
  return buildPrompt(
    ...
    'Напиши сильный AI-разбор по данным двух людей. Имена бери из context.people.subject.name и context.people.partner.name. Это не серверный астрологический расчёт: используй только переданный контекст и не приписывай системе факты, которых в нём нет.',
    input,
    '- Не вычисляй и не выдумывай положения планет, дома, аспекты, градусы и орбы. Если context.people.*.savedNatalSnapshot передан, используй только перечисленные в нём положения по знакам и ничего не достраивай. Если переданы только дата рождения или знак, дай честный общий разбор без заявлений о точной натальной совместимости. Время и место рождения сами по себе не дают права заявлять точные астрологические факты. Пол используй только для естественной грамматики, без гендерных стереотипов. Не показывай в пользовательском тексте технические данные, внутренние ограничения, названия полей и астрологический жаргон. Не используй термин «синастрия» в пользовательском тексте. В context.relationship передан тип связи: любовь, дружба, работа или семья. Разбирай только его; не добавляй романтическое притяжение в дружбу, работу или семейный контекст. summary — один прямой, запоминающийся вывод без оценки «хорошая» или «плохая» пара. generalTheme раскрывает главный сценарий общения и совместных решений. attraction описывает романтическую близость только для любви; для дружбы, работы и семьи раскрывает соответствующий вид интереса и притяжения. difficulties показывает один конкретный повторяющийся сбой без драматизации. recommendations — ровно три разных практичных действия без коучинговой воды. potential — реалистичный вывод о том, что поможет связи развиваться. compatibilityScore — редакционный показатель от 0 до 100, а не научное измерение; согласуй его с тоном разбора, но не объясняй и не называй его точным расчётом. Каждый раздел должен давать новый узнаваемый эпизод и не пересказывать предыдущий. Показывай обычные разговоры, реакции, распределение инициативы, бытовые решения и характерные фразы. Используй имена естественно, без приветствия. Обращайся уверенно на «ты». Не ставь диагнозы, не выноси приговор отношениям, не обещай брак, разрыв, измену или карьерный результат. Текст должен хотеться дочитать и узнавать в нём свою связь за счёт точности сцен, но никаких рекламных призывов сравнить ещё кого-либо.',
  );
}
