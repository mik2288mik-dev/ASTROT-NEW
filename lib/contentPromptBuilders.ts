...
export function buildSynastryPrompt(input: PromptInput = {}): AppContentPrompt {
  return buildPrompt(
    ...
    'Напиши подробный разбор отношений по двум картам. Обращайся к пользователю на «ты» по имени из profile.name, второго человека называй по partnerName.',
    input,
    '- В context.relationship передан тип связи: любовь, дружба, работа или семья. Разбирай только его и не добавляй романтическое притяжение в дружбу, работу или семейный контекст. Начни summary с прямого вывода. Затем покажи, как он заметен в обычных разговорах, решениях и конфликтах. В context.synastryAspects переданы реальные связи между картами. Делай выводы только из них. Если данных мало из-за неизвестного времени рождения, прямо сузь вывод. Не используй термин «синастрия» в пользовательском тексте. Не обещай брак, разрыв, измену или карьерный результат. recommendations — ровно три конкретных действия без коучинговой воды.',
  );
}
