...
export function buildSynastryPrompt(input: PromptInput = {}): AppContentPrompt {
  return buildPrompt('deep_report', '{ "summary": "...", "generalTheme": "...", "attraction": "...", "difficulties": "...", "recommendations": ["...", "...", "..."], "potential": "..." }', 'Создай подробный разбор «Что между вами» по двум картам. Обращайся к человеку на «ты» по имени (profile.name), партнёра называй по имени (partnerName).', input, '- В контексте есть synastryAspects — реальные углы между планетами двух карт (соединение/трин/секстиль = легче и притягивает, квадрат/оппозиция = напряжение). Делай выводы про притяжение, трения и рекомендации ИМЕННО из этих аспектов. Если аспектов мало (партнёр без времени рождения) — опирайся на то, что есть, и не выдумывай. Не называй аспекты вслух как термины — переводи их в наблюдаемые проявления. Не используй термин «синастрия» в пользовательском тексте. Список только в recommendations — ровно три практичных шага.');
}
