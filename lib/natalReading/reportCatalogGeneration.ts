...
export const NATAL_REPORT_NARRATIVE_FOCI: Record<NatalReportCategoryKey, readonly string[]> = {
  main: ['initiative', 'decisions', 'communication', 'closeness', 'curiosity', 'ease', 'pace', 'first_impression'],
  character: ['starting', 'deciding', 'changing_course', 'persisting', 'curiosity', 'ease', 'preference', 'pace'],
  love: ['interest', 'approach', 'affection', 'shared_time', 'closeness', 'reliability', 'choice', 'attraction'],
  communication: ['first_contact', 'explaining', 'listening', 'humour', 'directness', 'requests', 'reconnecting', 'pace'],
  work: ['starting', 'focus', 'pace', 'finishing', 'teamwork', 'solo_work', 'responsibility', 'interest'],
  money: ['small_purchases', 'large_choices', 'pleasure', 'saving', 'pricing', 'shared_money', 'independence', 'priorities'],
};
...
- Начни с одного узнаваемого вывода, который отличает именно эту карту. Дальше развивай только те условия и проявления, которые подтверждают выбранные факты. Не придумывай обязательную обратную сторону, проблему или конфликт ради контраста. Связывай наблюдения, не составляй перечень качеств.
...
- Open with one recognizable conclusion specific to this chart. Develop only the conditions and expressions supported by the selected evidence. Do not invent a mandatory downside, problem, or conflict merely to create contrast. Connect observations instead of listing traits.
...
MAIN READING ANCHOR has already been read. It preserves the same person but is not an outline for this chapter. Use only the compact topic-specific evidence supplied below. Write the required number of paragraphs from the strongest supported ideas; when there are fewer distinct ideas than paragraphs, deepen a real idea through a genuinely different condition instead of inventing another trait. Open with the most substantial one. Do not reword each anchor paragraph by adding “at work”, “in relationships”, or another category label.
...
export function buildNatalReportAnswerPrompt(input: {
  ...
  const task = input.language === 'ru'
    ? `Ответь только на этот вопрос по компактному набору фактов, уже отобранных именно для него. Дай самый сильный вывод в первом абзаце, затем 2–4 коротких абзаца с обычными ситуациями. Оговорку добавляй только при реальном основании. Не переносись в соседние темы и не добавляй конфликт, давление, контроль, риск или раздражение, если сам вопрос не про это и выбранные факты этого прямо не требуют. Если сам вопрос про неприятную тему, не усиливай вывод: по мягкому основанию пиши «может раздражать / может не нравиться», а не превращай это в обязательный конфликт или проблему. Раскрой уже показанный preview, но не копируй его дословно. Не давай советов.`
    : `Answer only this question from the compact evidence already selected for it. Give the strongest conclusion in the first paragraph, then 2–4 short paragraphs with ordinary situations. Add a qualification only when directly supported. Do not drift into neighbouring themes or introduce conflict, pressure, control, risk, or irritation unless the question is about them and the selected evidence directly requires it. If the question itself is negative, do not intensify the claim: soft evidence supports “may irritate / may feel uncomfortable”, not an automatic conflict or problem. Expand the existing preview without copying it verbatim. Give no advice.`;
  return `${task}
...
function narrativeValidationIssues(
  ...
  const minimumDistinctFocus = Math.min(2, summary.length);
  const maximumPerFocus = Math.max(2, Math.ceil(summary.length / 2));
  if (
    focusCounts.size < minimumDistinctFocus
    || [...focusCounts.values()].some((count) => count > maximumPerFocus)
  ) {
...
export async function generateNatalReportCategoryPack(input: {
  ...
  for (let attempt = 1; attempt <= NATAL_REPORT_SEMANTIC_ATTEMPTS; attempt += 1) {
    ...
    // One writer call per semantic attempt. A second call happens only when
    // server validation rejects the previous candidate and asks for repair.
    const edited = result;
