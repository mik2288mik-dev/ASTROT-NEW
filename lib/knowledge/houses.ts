...
export const HOUSE_TOPICS = [
  {
    ...
    copy: {
      ru: {
        ...
        summary: 'Астрологические дома — двенадцать частей круга натальной карты. Каждый дом связан с отдельной группой жизненных вопросов, а его границы рассчитываются для конкретного времени и места рождения.',
        sections: [
          { title: 'Как устроен круг домов', paragraphs: ['Границу, с которой начинается дом, называют куспидом. Планета указывает на тему, знак описывает, как выражается её значение, а дом показывает, к каким жизненным вопросам относят эту тему при чтении карты.'] },
          { title: 'Почему нужно время рождения', paragraphs: ['Сетка домов зависит от местного горизонта, а его положение меняется вместе со временем и местом наблюдения. Поэтому дома рассчитывают по времени и месту рождения. Если время неизвестно, дома нельзя выдавать за точную личную часть карты.'] },
        ],
        shortAnswer: 'Дома — это двенадцать частей круга натальной карты, связанные с разными жизненными вопросами; для их надёжного расчёта нужны время и место рождения.',
      },
      en: {
