...
export const SYNTHESIS_TOPICS = [
  {
    ...
    keywords: {
      ru: ['планета', 'знак', 'функция', 'стиль действия'],
      en: ['planet', 'sign', 'function', 'style of action'],
    },
    copy: {
      ru: {
        title: 'Планета в знаке',
        summary: 'Планета обозначает тему, а знак описывает привычный способ обращаться с ней.',
        sections: [
          {
        ...
        shortAnswer: 'Планета отвечает на вопрос «что», знак на вопрос «как», а остальная карта уточняет контекст.',
      },
      en: {
  ...
  },
  {
    ...
    copy: {
      ru: {
        ...
        summary: 'Дом показывает, в какой области жизни функция планеты чаще становится заметной и требует внимания.',
        sections: [
          {
            title: 'Где разворачивается тема',
            paragraphs: [
              'Если знак описывает способ действия, дом задаёт его жизненный контекст. Например, Меркурий остаётся связан с мышлением и обменом информацией, а дом уточняет, где эти качества особенно важны: в учёбе, работе, близких связях или другой области.',
            ],
          },
          {
  ...
  },
  {
    ...
    copy: {
      ru: {
        ...
        shortAnswer: 'Аспект описывает взаимодействие двух функций карты, а не выносит им оценку.',
      },
      en: {
