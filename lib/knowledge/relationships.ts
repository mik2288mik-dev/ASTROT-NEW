...
export const RELATIONSHIP_TOPICS = [
  {
    ...
    copy: {
      ru: {
        ...
        summary: 'Совместимость по знакам — это сравнение только двух солнечных знаков. Оно показывает общие сходства и различия, тогда как две полные натальные карты содержат гораздо больше данных.',
        sections: [
          {
            title: 'Что сравнивают',
            paragraphs: [
              'Солнечный знак — знак, в котором находилось Солнце при рождении; обычно его можно определить по дате рождения. Сравнение двух таких знаков описывает лишь общие различия в самовыражении и способах принимать решения.',
            ],
          },
          {
            title: 'Граница метода',
            paragraphs: [
              'Полная натальная карта содержит положения Луны, Венеры, Марса, Меркурия и других точек. Аспекты между двумя картами могут показать совсем другие сходства и различия, поэтому сравнение знаков остаётся общим, а не личным разбором пары.',
            ],
          },
        ],
        shortAnswer: 'Совместимость по знакам сравнивает только два солнечных знака и не описывает отношения или две полные натальные карты.',
      },
      en: {
  ...
  },
  {
    ...
    copy: {
      ru: {
        ...
        summary: 'Совместимость по двум картам — это сравнение положений планет и аспектов между двумя натальными картами. Такое сравнение включает больше данных, чем два солнечных знака.',
        sections: [
          {
            title: 'Что входит в сравнение',
            paragraphs: [
              'Обычно сопоставляют положения Солнца, Луны, Меркурия, Венеры и Марса, а также аспекты — связи между планетами двух карт. Углы карты и дома можно сравнивать только при надёжном времени рождения обоих людей.',
            ],
          },
          {
            title: 'Что остаётся за картой',
            paragraphs: [
              'Даже подробное сравнение не знает историю отношений, договорённости и реальные поступки. Оно описывает отдельные точки сходства и различия, но не оценивает пару одной цифрой.',
            ],
          },
        ],
        shortAnswer: 'Сравнение двух натальных карт учитывает больше данных, чем два солнечных знака, но не предсказывает будущее отношений.',
      },
      en: {
  ...
  },
  {
    ...
    copy: {
      ru: {
        ...
        summary: 'Синастрия — астрологический метод сравнения двух натальных карт. Планеты остаются в своих исходных картах, а программа рассчитывает углы между точками одного человека и точками другого.',
        sections: [
          {
            title: 'Что это на самом деле',
            kind: 'definition',
            paragraphs: [
              'Каждую натальную карту сначала рассчитывают отдельно по её дате, времени и месту. Затем два набора координат накладывают на один круг или показывают рядом. Исходные положения не усредняются и не создают третьего человека.',
            ],
          },
          {
            title: 'Как рассчитывают связи',
            kind: 'calculation',
            paragraphs: [
              'Программа ищет межкартовые аспекты: например, угол между Луной первой карты и Венерой второй. Также астрологи могут смотреть, в какие дома одной карты попадают планеты другой. Точность времени рождения важна для домов и углов.',
            ],
          },
          {
            title: 'Зачем это используют в астрологии',
            kind: 'astrology',
            paragraphs: [
              'Синастрию используют, чтобы обсуждать возможные точки лёгкого контакта, непонимания, притяжения и взаимного влияния. Смысл строится из конкретных планет, вида аспекта, орбиса и контекста обеих карт.',
            ],
          },
          {
            title: 'Синастрия и композит',
            kind: 'confusion',
            paragraphs: [
              'Синастрия сравнивает две реальные натальные карты. Композит создаёт одну расчётную карту из средних точек между одноимёнными положениями. Поэтому одинаковый вопрос эти методы показывают разным способом.',
            ],
          },
          {
            ...
            kind: 'confusion',
            paragraphs: [
              'Синастрия не сводится к проценту и не делит пары на правильные и неправильные. Один квадрат не отменяет другие связи, а много тригонов не гарантирует взаимность, безопасность и уважение. Решения об отношениях нельзя передавать одной карте.',
            ],
          },
          {
            title: 'Откуда появился метод',
            kind: 'history',
            depth: 'deep',
            paragraphs: [
              'Сравнение карт развивалось постепенно внутри астрологической традиции; у синастрии нет одного известного автора. Современные программы сделали расчёт больших наборов межкартовых аспектов быстрым, но сами принципы сопоставления старше цифровых карт.',
            ],
          },
        ],
        shortAnswer: 'Синастрия накладывает две карты и сравнивает их точки. Она описывает возможную динамику, но не выставляет отношениям оценку и не предсказывает итог.',
      },
      en: {
    ...
    relatedTopicIds: ['two-chart-compatibility', 'interchart-aspects', 'composite-chart', 'aspects-overview', 'compatibility-not-fate'],
  },
  {
    ...
    copy: {
      ru: {
        ...
        summary: 'Луну в совместимости рассматривают по её положению в каждой натальной карте и аспектам между двумя картами. В астрологии эти данные связывают с эмоциональными реакциями и способами заботы.',
        sections: [
          {
            title: 'Повседневная близость',
            paragraphs: [
              'Натальная Луна относится к тому, как человек просит поддержки, реагирует на напряжение и понимает заботу. Совпадение знаков Луны — лишь один частный случай; разные знаки не означают несовместимость.',
            ],
          },
          {
            title: 'Луна в сравнении карт',
            paragraphs: [
              'Аспекты показывают связи Луны с планетами и точками карты другого человека. В астрологии их относят к эмоциональным реакциям, но читают вместе с Венерой, Марсом, Меркурием и другими частями обеих карт.',
            ],
          },
        ],
        shortAnswer: 'Луна в сравнении карт описывает эмоциональные реакции и способы заботы, но её положение и аспекты не определяют совместимость.',
      },
      en: {
  ...
  },
  {
    ...
    copy: {
      ru: {
        ...
        summary: 'Венеру в отношениях рассматривают по её положению в натальной карте и аспектам с точками другой карты. В астрологии эти данные относят к симпатии, вкусу и способу показывать расположение.',
        sections: [
          {
            title: 'Что приятно получать и давать',
            paragraphs: [
              'В астрологии знак Венеры связывают с тем, какие жесты человек считает тёплыми, красивыми и уместными. Это относится не только к романтике, но и к дружбе, общению и совместному досугу.',
            ],
          },
          {
            title: 'Разные способы показывать внимание',
            paragraphs: [
              'Разные положения Венеры не означают несовместимость. В астрологическом описании один человек может чаще показывать расположение словами, другой — делами или временем вместе. Ни одно положение не доказывает глубину чувств.',
            ],
          },
        ],
        shortAnswer: 'Положение Венеры в натальной карте связывают со способом показывать симпатию, но оно не измеряет глубину отношений.',
      },
      en: {
  ...
  },
  {
    ...
    copy: {
      ru: {
        ...
        summary: 'Марс в отношениях рассматривают по его положению в натальной карте и аспектам с точками другой карты. В астрологии эти данные относят к инициативе, прямому желанию и действиям при несогласии.',
        sections: [
          {
            title: 'Темп действия',
            paragraphs: [
              'В астрологии знак Марса связывают с тем, как человек начинает дело, добивается ответа и реагирует на препятствие. В сравнении отношений это относят к инициативе, физическому влечению и поведению в споре.',
            ],
          },
          {
            title: 'Напряжение не равно конфликту',
            paragraphs: [
              'Связи Марса могут описывать сильный интерес или разницу темпов. Они не заставляют людей ссориться: реальное поведение нельзя вывести из одной позиции планеты или одного аспекта.',
            ],
          },
        ],
        shortAnswer: 'Положение Марса в натальной карте связывают с инициативой и действиями при несогласии, но оно не предсказывает ссоры.',
      },
      en: {
  ...
  },
  {
    ...
    copy: {
      ru: {
        ...
        summary: 'Меркурий в отношениях рассматривают по его положению в натальной карте и аспектам с точками другой карты. В астрологии эти данные относят к способу думать, объяснять и воспринимать ответ.',
        sections: [
          {
            title: 'Разные языки разговора',
            paragraphs: [
              'В астрологическом описании один человек говорит коротко и прямо, другой сначала собирает детали или проверяет оттенки смысла. Разные положения Меркурия не означают, что один способ умнее другого.',
            ],
          },
          {
            title: 'Связи между картами',
            paragraphs: [
              'Аспекты показывают связи Меркурия с планетами и точками карты другого человека. Их могут связывать с более лёгким пониманием или частой потребностью в уточнениях, но они не определяют умение людей слушать и договариваться.',
            ],
          },
        ],
        shortAnswer: 'Положение Меркурия в натальной карте связывают со стилем общения, но оно не определяет, умеют ли люди договариваться.',
      },
      en: {
  ...
  },
  {
    ...
    copy: {
      ru: {
        ...
        summary: 'Межкартовый аспект — это связь между планетой или точкой в карте одного человека и точкой в карте другого.',
        sections: [
          {
            title: 'Что именно сравнивают',
            paragraphs: [
              'Программа сравнивает положения двух точек и определяет вид аспекта. Орб показывает, насколько точно они заняли нужное для него положение. Затем учитывают сами планеты: связь Луны и Меркурия читают иначе, чем связь Марса и Венеры.',
            ],
          },
          {
        ...
        shortAnswer: 'Межкартовый аспект описывает одну рассчитанную связь между двумя натальными картами, а не отношения целиком.',
      },
      en: {
  ...
  },
  {
    ...
    copy: {
      ru: {
        title: 'Совместимость не решает, будут ли люди вместе',
        summary: 'Астрологическая совместимость — это сравнение знаков, планет и аспектов двух карт. Она описывает отдельные сходства и различия, но не предсказывает итог отношений.',
        sections: [
          {
            title: 'Что даёт сравнение',
            paragraphs: [
              'Сравнение показывает, где астрологические описания двух людей похожи, а где различаются способы общения и действия. Это описание отдельных связей между картами, а не оценка того, стоит ли людям быть вместе.',
            ],
          },
          {
        ...
        shortAnswer: 'Астрологическая совместимость описывает отдельные сходства и различия двух карт, но не предсказывает решения или качество отношений.',
      },
      en: {
