...
export const ANGLE_TOPICS = [
  ...
  },
  {
    ...
    category: 'angles',
    aliases: {
      ru: ['асцендент', 'асцедент', 'asc', 'ac', 'восходящий знак', 'rising', 'rising sign'],
    ...
    copy: {
      ru: {
        title: 'Асцендент (ASC)',
        summary: 'Асцендент — точка зодиака, которая в конкретный момент находится на восточном горизонте. В натальной карте это положение рассчитывают для времени и места рождения и обозначают ASC или AC.',
        sections: [
          {
            title: 'Что это на самом деле',
            kind: 'fact',
            paragraphs: [
              'Представьте две линии на небесной сфере: горизонт делит видимую и невидимую части неба, а эклиптика показывает видимый годовой путь Солнца. Восточная точка их пересечения и есть Асцендент. Это геометрическая точка, а не планета и не отдельное небесное тело.',
            ],
          },
          {
            title: 'Как его определяют',
            kind: 'calculation',
            paragraphs: [
              'Программа знает дату, точное время, широту и долготу места. По ним она вычисляет местный горизонт и находит его пересечение с эклиптикой. Результат записывают знаком и градусом, например «ASC в пятнадцати градусах Тельца».',
            ],
          },
          {
            title: 'Почему важны время и место',
            kind: 'mechanism',
            paragraphs: [
              'Земля вращается, поэтому восточный горизонт быстро пересекает разные участки зодиака. За сутки Асцендент проходит весь круг; скорость прохождения отдельных знаков не одинакова и зависит от географической широты. Даже люди, родившиеся одновременно в далёких городах, могут получить разные Асценденты.',
            ],
          },
          {
            title: 'Зачем это используется в астрологии',
            kind: 'astrology',
            paragraphs: [
              'Асцендент служит началом первого дома и одной из четырёх главных опор карты. В астрологии его знак связывают со способом входить в новую ситуацию, первой заметной реакцией и внешней манерой проявляться. Планеты рядом с ASC и аспекты к нему уточняют трактовку.',
            ],
          },
          {
            title: 'Часто путают',
            kind: 'confusion',
            paragraphs: [
              'Асцендент не равен знаку Солнца. Солнце — реальный объект с рассчитанной долготой, ASC — пересечение горизонта и эклиптики. Формула «социальная маска» слишком узкая: Асцендент используют не только для впечатления на других, но и как начало системы домов.',
            ],
          },
          {
            ...
            depth: 'deep',
            paragraphs: [
              'Слово происходит от латинского ascendere — «восходить». Противоположная западная точка называется Десцендентом; ещё две главные точки карты — MC и IC.',
            ],
          },
        ],
        shortAnswer: 'Асцендент — восточное пересечение горизонта с зодиаком. Поэтому без достаточно точного времени и места его нельзя надёжно определить.',
      },
      en: {
        title: 'Ascendant (ASC)',
    ...
    relatedTopicIds: ['zodiac-geometry', 'natal-chart-basics', 'house-1', 'descendant', 'midheaven', 'imum-coeli'],
    diagram: 'ascendant',
    sourceIds: ['astro-ascendant'],
  },
  {
    id: 'descendant',
    category: 'angles',
