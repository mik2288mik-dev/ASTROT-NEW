...
import { buildPersonalDailyPrompt, parseModelJson } from './contentPromptBuilders';
...
function buildDaypartFallback(
  ...
  const ruCopy: Record<ForecastDaypartSlot, Omit<ForecastDaypartReading, 'date' | 'slot'>> = {
    ...
    day: {
      headline: 'Сегодня выигрывает точность, а не суета',
      summary: 'Днём легко перепутать занятость с движением: задач много, а к вечеру непонятно, что реально сдвинулось. Сила этого дня — выбрать одно дело, которое доведёшь до конца, и держаться его, не давая мелочам растащить внимание.',
      focus: 'Держись ближе к тому, что даёт результат, а не к ощущению, что ты весь день занят. Одно доведённое до конца дело сегодня важнее пяти красиво начатых.',
      relationships: 'Напряжение в общении сегодня чаще рождается из быстрого тона и недосказанности, чем из самой темы. Если что-то задело — скажи прямо и спокойно, не копи до вечера и не проверяй человека молчанием.',
      money: 'Денежные и практические решения принимай по понятному критерию, а не на импульсе и не под чужим «надо срочно». Сначала проверь условия — потом говори «да».',
      guidance: 'Если чувствуешь перегруз — не ускоряйся на автомате. Остановись на минуту, верни себе опору, переведи дух, и только потом выбирай следующий шаг. Спокойный темп сегодня обгонит рывок на нервах.',
    },
    evening: {
  ...
  const enCopy: Record<ForecastDaypartSlot, Omit<ForecastDaypartReading, 'date' | 'slot'>> = {
    ...
    day: {
      headline: 'Today rewards precision, not busywork',
      summary: 'It is easy to confuse being busy with moving forward: lots of tasks, yet by evening it is unclear what actually shifted. The strength of this day is picking one thing to finish and holding to it instead of letting small stuff pull you apart.',
      focus: 'Stay close to what creates a real result, not just the feeling of being busy. One thing carried to completion today matters more than five nicely started.',
      relationships: 'Tension in conversations today grows more from a rushed tone and things left unsaid than from the topic itself. If something stung, say it plainly and calmly — do not store it up for the evening or test the other person with silence.',
      money: 'Make money and practical decisions by a clear criterion, not on impulse or someone else’s “need it now”. Check the terms first, then say yes.',
      guidance: 'If you feel overloaded, do not speed up on autopilot. Pause, regain your footing, take a breath, and only then choose the next step. A calm pace today will outrun a nervous sprint.',
    },
    evening: {
...
export async function generateFreeDailyForecast(
  ...
  try {
    ...
    const parsed = parseModelJson<DailyForecastV2AIResponse & { main?: string; relationships?: string; action?: string; why?: string }>(completion.choices[0]?.message?.content, {} as DailyForecastV2AIResponse);
...
export async function generatePremiumDaypartForecast(
  ...
  try {
    ...
    const completion = await openai.chat.completions.create(buildOpenAIChatParams(model, {
      messages: [
        { role: 'system', content: getAppSystemPrompt(lang) },
