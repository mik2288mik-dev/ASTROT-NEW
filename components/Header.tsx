...
interface HeaderProps {
    ...
    onBack: () => void;
    onOpenWallet: () => void;
    /** Compact single-row hub bar (Variant 4 home); profile lives on dashboard body */
    compactHub?: boolean;
...
export const Header: React.FC<HeaderProps> = ({
    ...
    return (
        ...
        >
            <div className={`px-4 ${compactHub && isHub ? 'pb-2 pt-0.5' : 'pt-1 pb-2.5'}`}>
                {compactHub && isHub ? (
                    <div className="flex h-10 items-center justify-between gap-2">
                        <LumiaLogo variant="row" className="min-w-0 shrink scale-[0.82] sm:scale-90" />
                        <button
                            type="button"
                            onClick={onOpenWallet}
                            className="flex shrink-0 items-center gap-1.5 rounded-full border border-astro-border/50 bg-astro-card/70 px-2.5 py-1 text-[11px] font-semibold text-astro-text"
                        >
                        ...
                        <button
                            ...
                            className="relative flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full ring-1 ring-astro-text/10"
                            aria-label={getText(profile.language, 'settings.title')}
                        >
                            {hubAvatarUrl ? (
                                <Image
                                    src={hubAvatarUrl}
                                    ...
                                    unoptimized={hubAvatarUrl.startsWith('http')}
                                />
                            ) : (
                                <span className="text-xs font-semibold text-astro-subtext">
