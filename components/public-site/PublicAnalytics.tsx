...
function analytics(): AnalyticsWindow {
  ...
  if (yandexMetrikaId && !analyticsWindow.ym) {
    ...
    }) as AnalyticsWindow['ym'] & { a: unknown[][] };
    queuedYm.a = [];
...
function sendPageView(url: string) {
  ...
  if (ga4MeasurementId) {
    analyticsWindow.gtag?.('config', ga4MeasurementId, {
...
/** Browser-only public-site metrics. It is inert until an ID is supplied at build time. */
export function PublicAnalytics() {
  ...
  useEffect(() => {
    ...
    const readConsent = () => {
      setAnalyticsConsent(window.localStorage.getItem(consentStorageKey) === 'granted');
    };
    const openConsent = () => {
  ...
  if (analyticsConsent === null) {
    return (
      <aside className={styles.analyticsConsent} role="dialog" aria-label="Настройки аналитики">
        <p>
          Мы используем Яндекс Метрику и Google Analytics, чтобы понимать посещаемость и
          улучшать сайт. В Метрике включён Вебвизор. Подробнее — в{' '}
          <a href="/privacy">политике конфиденциальности</a>.
        </p>
        <div className={styles.analyticsConsentActions}>
          <button type="button" onClick={() => chooseConsent('granted')}>Разрешить аналитику</button>
          <button type="button" onClick={() => chooseConsent('denied')}>Только необходимое</button>
        </div>
      </aside>
    );
  }
