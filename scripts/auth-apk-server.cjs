...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/nebo-auth-test.apk') {
    sendFile(req, res, apkPath, 'application/vnd.android.package-archive', 'NEBO-natal-admin-test.apk');
    return;
  }
  if (req.url === '/fingerprints.txt') {
    sendFile(req, res, fingerprintPath, 'text/plain; charset=utf-8', 'NEBO-natal-admin-test-fingerprints.txt');
    return;
  }
  if (req.url === '/' || req.url === '/index.html') {
    const body = [
      'NEBO natal admin Android test build',
      '',
...
server.listen(port, '0.0.0.0', () => {
  console.log(`NEBO natal-admin APK server listening on ${port}`);
});