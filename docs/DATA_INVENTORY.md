...
| Session token, IP address, user agent, diagnostics, and security events | Authentication, abuse prevention, reliability, and incident analysis | The application backend handles sessions; verify its hosting logs and retention separately from the public website | Sessions are revoked; policy text must state the verified log and backup retention |
