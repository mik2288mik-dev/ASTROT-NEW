...
| Purchase/subscription information | RuStore Pay SDK/callback/Public API → payment APIs | `store_purchases`, provider-event inbox, Premium entitlements/history | RuStore (`ООО «Много приложений»`) | User-linked ledger and entitlements delete/cascade; event payload and external purchase ID are scrubbed. RuStore retains its own transaction records independently. The target first RuStore release includes Premium after RuStore Console, production environment and sandbox validation are complete; monetisation remains disabled until those gates pass. |
