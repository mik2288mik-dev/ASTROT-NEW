...
function preparedCardFromPulse(pulse: TodayPulse | null): PreparedDailyCard {
  ...
  return {
    theme: peak?.title || point?.title || 'Главный ориентир дня',
...
const FALLBACK_COPY: Record<RetentionNotificationType, { title: string; body: string; button: string }> = {
  ...
  natal_free: {
    title: 'Твоя общая карта готова',
    body: 'Мы разобрали её простым языком: поведение, отношения, работу и то, что может мешать.',
    button: 'Открыть мою карту',
  },
  love: {
    title: 'Подсказка по отношениям',
    body: 'Сегодня лучше смотреть не на слова, а на повторяющееся поведение. В разделе Любовь есть подсказка на день.',
  ...
  premium: {
    title: 'Полная карта даёт больше конкретики',
    body: 'В бесплатной карте показано главное. В полной версии можно разобрать отношения, деньги и повторяющиеся сценарии.',
    button: 'Открыть полную карту',
  },
  inactive_2d: {
    title: 'Карта дня обновилась',
    body: 'Можно быстро посмотреть, что сегодня лучше делать, а где не давить. Без длинного текста.',
    button: 'Открыть Lumia',
  },
  inactive_7d: {
    title: 'Есть новый разбор дня',
    body: 'Неделя могла пройти на автомате. Открой Lumia и посмотри, где сейчас лучше вернуть внимание.',
    button: 'Вернуться',
  },
  inactive_14d: {
    title: 'Можно начать с одного короткого разбора',
    body: 'Открой карту дня: там один главный ориентир и конкретный совет, без лишнего шума.',
    button: 'Открыть разбор',
  },
  birth_data_missing: {
    title: 'Нужны данные рождения',
    body: 'Без даты и места рождения Lumia не сможет собрать личную карту. Заполни данные и увидишь первый разбор.',
