...
type MeouDocumentProps = DocumentInitialProps & { publicDocument: boolean };

const PUBLIC_ROUTES = new Set(['/404','/delete-account','/personal-data-consent','/privacy','/requisites','/site','/support','/terms']);
const PUBLIC_ROUTE_PREFIXES = ['/goroskop','/lichnyy-goroskop','/natalnaya-karta','/sovmestimost'];
function isPublicRoute(pathname: string): boolean { return PUBLIC_ROUTES.has(pathname) || PUBLIC_ROUTE_PREFIXES.some((prefix) => pathname === prefix || pathname.startsWith(`${prefix}/`)); }
...
export default class MeouDocument extends Document<MeouDocumentProps> {
  ...
  render() {
    ...
    const isMobileBuild = process.env.NEXT_PUBLIC_MOBILE_BUILD === '1' || process.env.MOBILE_BUILD === '1';
    const loadTelegramAppDependencies = !publicDocument && !isUiPreviewBuild && !isMobileBuild;

    return (
      <Html lang="ru" className="antialiased">
        <Head>
          <meta name="theme-color" content={publicDocument ? '#070707' : '#fbfaf8'} />
